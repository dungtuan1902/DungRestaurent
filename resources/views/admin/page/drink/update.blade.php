...
@section('content_fluid')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form class="row g-3" method="POST" action="{{ route('admin.drink.update', ['id' => $drink->id]) }}"
                enctype="multipart/form-data">
                @csrf
                <div class="col-md-4">
                    <label for="inputName" class="form-label">Name</label>
                    <input type="text" class="form-control" name="name" id="inputName" value="{{ $drink->name }}">
                    @if ($errors->has('name'))
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" name="price" id="inputPrice" value="{{ $drink->price }}">
                    @if ($errors->has('price'))
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="inputFoodType" name="drink_type_id" aria-label="Default select example">
                        @foreach ($drinktype as $d)
                            <option value="{{ $d->id }}" {{ $drink->drink_type_id == $d->id ? 'selected' : '' }}>
                                {{ $d->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    <img id="image_preview" style="width: 350px ; height:200px" src="{{ Storage::url($drink->image) }}"
                        alt="Loading....">
                </div>
                <div class="col-md-12">
                    <div class="form-floating">
                        <textarea class="form-control" placeholder="Leave a comment here" name="description" id="floatingTextarea">{{ $drink->description }}</textarea>
